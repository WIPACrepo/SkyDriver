...
class TMSStatusHandler(BaseSkyDriverHandler):  # pylint: disable=W0223
    """Handles persisting statuses of clusters."""

    ROUTE = r"/tms/cluster/(?P<cluster_uuid>\w+)/status$"

    @service_account_auth(roles=[SKYMAP_SCANNER_ACCT])  # type: ignore
    async def put(self, cluster_uuid: str) -> None:
        """Put (persist) a scan's result."""
        skyscan_result: dict[str, Any] = self.get_argument(
            "skyscan_result",
            type=dict,
